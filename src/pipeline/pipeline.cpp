...
namespace BlockyVulkan {
    Pipeline::Pipeline( BlockyVulkan::Device &device,
    ...
    void Pipeline::CreateGraphicsPipeline( const std::string &vertFilePath,
        const std::string &fragFilePath,
        const PipelineConfigInfo &configInfo ) {
        assert( configInfo.pipelineLayout != VK_NULL_HANDLE && "Cannot create graphics pipeline: No pipelineLayout provided! (at configInfo)" );
        assert( configInfo.renderPass != VK_NULL_HANDLE && "Cannot create graphics pipeline: No renderPass provided! (at configInfo)" );
...
PipelineConfigInfo Pipeline::DefaultPipelineConfigInfo(uint32_t width,
  ...
  // Viewport
  configInfo.viewport.x = .0f;
  configInfo.viewport.y = .0f;
  configInfo.viewport.width = static_cast<float>(width);
  configInfo.viewport.height = static_cast<float>(height);
  configInfo.viewport.minDepth = .0f;
  configInfo.viewport.maxDepth = .0f;
  ...
  configInfo.rasterizationInfo.depthBiasConstantFactor = .0f;  // Optional
  configInfo.rasterizationInfo.depthBiasClamp = .0f;           // Optional
  configInfo.rasterizationInfo.depthBiasSlopeFactor = .0f;     // Optional
  ...
  configInfo.depthStencilInfo.minDepthBounds = .0f;          // Optional
